...
import { NOTICES, QUICK_LINKS, type NoticeTag } from '@/pages/main/model/homeMock';
  ...
  return (
    <section className="w-full" aria-labelledby="home-notice-quick-heading">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)] lg:gap-5">
        ...
        <article className="rounded-2xl px-5 py-5 sm:px-6 sm:py-6">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
            {QUICK_LINKS.map((link) => (
              <Link
                key={link.id}
                to={link.to}
                className="flex flex-col items-center gap-2.5 rounded-xl border border-neutral-200 bg-white px-2 py-3 transition-colors hover:border-neutral-300"
              >
                <ImagePlaceholder className="aspect-square w-full max-w-[72px] rounded-lg sm:max-w-[180px]" />
                <span className="text-center text-[14px] font-medium text-neutral-800">{link.label}</span>
              </Link>
            ))}
          </div>
        </article>
